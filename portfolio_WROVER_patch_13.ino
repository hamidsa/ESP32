...
void manageWiFiMode() {
    ...
    // هر 5 ثانیه یکبار چک کن
    if (now - lastCheck < 5000) return;
    lastCheck = now;
    
    if (WiFi.status() == WL_CONNECTED) {
        if (apModeActive && apEnabled) {
            Serial.println("📡 WiFi connected → AP remains active (Hybrid Mode)");
            // حالت Hybrid: AP + STA
        ...
        } else if (apModeActive && !apEnabled) {
            Serial.println("📡 WiFi connected → Stopping AP (STA Only Mode)");
        ...
        if (!apModeActive && apEnabled) {
            Serial.println("📡 WiFi disconnected → Starting AP");
            startAPMode();
        }
...
void setup() {
    ...
    if (settings.networkCount > 0 && !apModeActive && !isConnectedToWiFi) {
    ...
    } else if (!apModeActive && !isConnectedToWiFi && apEnabled) {
        Serial.println("No WiFi networks configured or connection failed, starting AP mode");
        startAPMode();
    }
