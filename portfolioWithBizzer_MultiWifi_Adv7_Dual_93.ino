...
void showExitAlert(String title, String msg1, String msg2, String msg3, bool isProfit, float changePercent, float price) {
    ...
    // === اصلاح: پیدا کردن موقعیت اصلی از داده‌های crypto ===
    bool positionIsLong = true; // مقدار پیش‌فرض
    ...
    // روش 2: جستجوی با حذف پسوندها
    if (!foundPosition) {
        String searchSymbol = symbol;
        // حذف پسوندهای رایج
        if (searchSymbol.endsWith("USDT")) {
            searchSymbol = searchSymbol.substring(0, searchSymbol.length() - 4);
        } else if (searchSymbol.endsWith("_USDT")) {
            searchSymbol = searchSymbol.substring(0, searchSymbol.length() - 5);
        }
        
        for (int i = 0; i < cryptoCount; i++) {
            String dataSymbol = String(cryptoData[i].symbol);
            if (dataSymbol.startsWith(searchSymbol) || dataSymbol.endsWith(searchSymbol)) {
                positionIsLong = cryptoData[i].isLong;
                entryPrice = cryptoData[i].entryPrice;
                lastAlertedPrice = cryptoData[i].exitAlertLastPrice;
                foundSymbol = String(cryptoData[i].symbol);
                foundPosition = true;
                Serial.println("  ✓ Found partial match: " + foundSymbol + " for " + symbol);
                break;
            }
        }
    }
    
    // روش 3: جستجوی بدون حساسیت به حروف
    if (!foundPosition) {
        String searchSymbol = symbol;
        ...
        for (int i = 0; i < cryptoCount; i++) {
            ...
            if (dataSymbol == searchSymbol || 
                dataSymbol.indexOf(searchSymbol) != -1 || 
                searchSymbol.indexOf(dataSymbol) != -1) {
                positionIsLong = cryptoData[i].isLong;
                entryPrice = cryptoData[i].entryPrice;
                lastAlertedPrice = cryptoData[i].exitAlertLastPrice;
                foundSymbol = String(cryptoData[i].symbol);
                foundPosition = true;
                Serial.println("  ✓ Found case-insensitive match: " + foundSymbol + " for " + symbol);
                break;
            }
        }
    }
    ...
    if (foundPosition) {
        ...
        // محاسبه P/L کل نسبت به نقطه ورود
        if (entryPrice > 0) {
            if (positionIsLong) {
                // LONG: (قیمت فعلی - قیمت ورود) / قیمت ورود * 100
                totalPnlPercent = ((price - entryPrice) / entryPrice) * 100;
            } else {
                // SHORT: (قیمت ورود - قیمت فعلی) / قیمت ورود * 100
        ...
        Serial.println("  Searching all available symbols:");
        for (int i = 0; i < min(10, cryptoCount); i++) {
            Serial.println("    - " + String(cryptoData[i].symbol) + " (" + String(cryptoData[i].isLong ? "LONG" : "SHORT") + ")");
        }
    ...
    // محاسبه isProfit بر اساس تغییر از آخرین الرت (همان logic قبلی)
    bool isProfitFromLastAlert = isProfit; // همان مقدار ورودی
    ...
    if (alertHistoryCount < MAX_ALERT_HISTORY) {
        ...
        // ذخیره P/L کل از نقطه ورود (نه تغییر از الرت قبلی)
        alert->pnlPercent = totalPnlPercent; // استفاده از P/L کل
        alert->alertPrice = price;
        
        // استفاده از موقعیت واقعی
        alert->isLong = positionIsLong;
        
        alert->isSevere = (fabs(totalPnlPercent) >= 10.0);
        alert->isProfit = isProfitFromLastAlert; // استفاده از isProfit از تغییر آخرین الرت
        alert->alertType = isProfitFromLastAlert ? 3 : 4; // 3 = PROFIT, 4 = LOSS
        
        // مشخص کردن مود
        alert->alertMode = 1; // 1 = Exit Mode
        
        // ========== اصلاح: ذخیره پیام تمیزتر ==========
        String positionType = positionIsLong ? "LONG" : "SHORT";
        String profitLossType = isProfitFromLastAlert ? "PROFIT" : "LOSS";
        
        // ساخت پیام با فرمت تمیز
        String alertMessage = "";
        
        // بخش 1: نوع الرت
        alertMessage += positionType + " " + profitLossType + " ALERT";
        
        // بخش 2: اطلاعات قیمت
        alertMessage += " | Current: " + formatPrice(price);
        
        // بخش 3: Last Alert Price (اگر موجود باشد)
        ...
        // بخش 4: Entry Price (اگر موجود باشد)
        if (entryPrice > 0) {
            alertMessage += " | Entry: " + formatPrice(entryPrice);
            
            // محاسبه P/L از نقطه ورود
...
void checkExitModeAlerts() {
    ...
    for (int i = 0; i < min(10, cryptoCount); i++) {
        ...
        if (cryptoData[i].exitAlertLastPrice > 0) {
            float change = fabs((cryptoData[i].currentPrice - cryptoData[i].exitAlertLastPrice) / 
            ...
            // محاسبه P/L از نقطه ورود
            if (cryptoData[i].entryPrice > 0) {
                float totalPnl;
                if (cryptoData[i].isLong) {
                    totalPnl = ((cryptoData[i].currentPrice - cryptoData[i].entryPrice) / cryptoData[i].entryPrice) * 100;
                } else {
                    totalPnl = ((cryptoData[i].entryPrice - cryptoData[i].currentPrice) / cryptoData[i].entryPrice) * 100;
                }
                Serial.print(" | Total P/L: ");
                Serial.print(totalPnl, 1);
                Serial.print("%");
            }
        }
        Serial.println();
    ...
    for (int i = 0; i < cryptoCount && !newAlertTriggered; i++) {
        ...
        if (priceChangePercent >= settings.exitAlertPercent) {
            ...
            // ارسال totalPnlPercent (P/L کل) به جای changePercent
...
void debugRawJSON(String jsonData) {
    ...
    // حل مشکل type mismatch با cast کردن
    ...
    // سعی کن JSON را parse کن و فیلدها را نمایش بده
    ...
    if (!error) {
        ...
        if (doc.containsKey("portfolio")) {
            ...
            if (portfolio.size() > 0) {
                JsonObject firstItem = portfolio[0];
                Serial.println("\n  First item fields:");
                int fieldCount = 0;
                for (JsonPair kv : firstItem) {
                    String key = String(kv.key().c_str());
                    JsonVariant value = kv.value();
                    String valueStr;
                    
                    if (value.is<float>()) {
                        valueStr = String(value.as<float>(), 6);
                    } else if (value.is<int>()) {
                        valueStr = String(value.as<int>());
                    } else if (value.is<bool>()) {
                        valueStr = String(value.as<bool>());
                    } else {
                        valueStr = value.as<String>();
                    }
                    
                    Serial.println("    " + key + ": " + valueStr);
                    fieldCount++;
                    
                    if (fieldCount >= 15) { // محدود کردن برای جلوگیری از overflow
                        Serial.println("    ... (more fields)");
                        break;
                    }
                }
                
                // بررسی فیلدهای موقعیت
                Serial.println("\n  Position fields check:");
                const char* positionFields[] = {"position", "position_side", "side", "quantity"};
                for (int i = 0; i < 4; i++) {
                    const char* field = positionFields[i];
                    if (firstItem.containsKey(field)) {
                        String value = firstItem[field].as<String>();
                        Serial.println("    ✓ " + String(field) + ": " + value);
                    } else {
                        Serial.println("    ✗ " + String(field) + ": NOT FOUND");
                    }
                }
            }
        } else {
        ...
        if (doc.containsKey("summary")) {
            Serial.println("✓ Summary field found");
        } else {
            Serial.println("✗ 'summary' field NOT FOUND");
        }
    } else {
...
String getExitModeData() {
    ...
    if (httpCode == 200) {
        ...
        // اضافه کردن دیباگ برای بررسی ساختار JSON
        debugRawJSON(result);
    ...
    for (int i = 0; i < cryptoCount; i++) {
        ...
        if (position.containsKey("position")) {
            ...
            if (posTypeStr == "SHORT" || posTypeStr == "SELL") {
                ...
                Serial.println("  ✓ Detected SHORT from 'position' field: " + String(symbol));
            } else if (posTypeStr == "LONG" || posTypeStr == "BUY") {
                isLongFromJSON = true;
                Serial.println("  ✓ Detected LONG from 'position' field: " + String(symbol));
            } else {
                Serial.println("  ? Unknown position type '" + posTypeStr + "' for " + String(symbol) + ", defaulting to LONG");
            }
            positionFound = true;
        }
        
        // اولویت ۲: فیلد position_side
        if (!positionFound && position.containsKey("position_side")) {
            ...
            if (sideStr == "SHORT") {
                isLongFromJSON = false;
                Serial.println("  ✓ Detected SHORT from 'position_side' field: " + String(symbol));
            } else if (sideStr == "LONG") {
                isLongFromJSON = true;
                Serial.println("  ✓ Detected LONG from 'position_side' field: " + String(symbol));
            } else {
                Serial.println("  ? Unknown position_side '" + sideStr + "' for " + String(symbol) + ", defaulting to LONG");
            }
            positionFound = true;
        }
        ...
        if (!positionFound && position.containsKey("side")) {
            ...
            if (sideStr == "SELL" || sideStr == "SHORT") {
                isLongFromJSON = false;
                Serial.println("  ✓ Detected SHORT from 'side' field: " + String(symbol));
            } else if (sideStr == "BUY" || sideStr == "LONG") {
                isLongFromJSON = true;
                Serial.println("  ✓ Detected LONG from 'side' field: " + String(symbol));
            } else {
                Serial.println("  ? Unknown side '" + sideStr + "' for " + String(symbol) + ", defaulting to LONG");
            }
            positionFound = true;
        }
        
        // اولویت ۴: فیلد quantity
        if (!positionFound && position.containsKey("quantity")) {
            float quantity = position["quantity"] | 0.0;
            if (quantity < 0) {
                isLongFromJSON = false;
                Serial.println("  ✓ Detected SHORT from negative quantity: " + String(symbol) + " (" + String(quantity) + ")");
            } else if (quantity > 0) {
                isLongFromJSON = true;
                Serial.println("  ✓ Detected LONG from positive quantity: " + String(symbol) + " (" + String(quantity) + ")");
            } else {
                Serial.println("  ? Zero quantity for " + String(symbol) + ", defaulting to LONG");
            }
            positionFound = true;
        }
        
        // اولویت ۵: فیلد pnl_percent
        if (!positionFound && position.containsKey("pnl_percent")) {
            float pnlPercent = position["pnl_percent"] | 0.0;
            // فقط برای اطلاعات
            Serial.println("  ⚠️ Position type not found for " + String(symbol) + 
                          ", P/L%: " + String(pnlPercent, 1) + 
                          "%, defaulting to LONG");
            isLongFromJSON = true;
        }
        ...
        if (!positionFound) {
            Serial.println("  ⚠️ No position fields found for " + String(symbol) + ", defaulting to LONG");
            isLongFromJSON = true;
        }
        // === پایان اصلاح ===
